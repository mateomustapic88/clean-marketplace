<template>
  <CleanerDashboard>
    <header class="cleaner-home__welcome"><div><DemoBadge type="profile" /><h1>{{ t('cleaner.dashboard.welcome', { name: profile?.firstName ?? authStore.user?.displayName ?? '' }) }}</h1><p>{{ t('cleaner.dashboard.description') }}</p></div><BaseButton :to="getAppRoute('cleanerJobs', locale)">{{ t('cleaner.dashboard.browse') }}</BaseButton></header>
