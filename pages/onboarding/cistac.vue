...
const toggleLanguage = (language: string) => draft.languages = draft.languages.includes(language) ? draft.languages.filter((item) => item !== language) : [...draft.languages, language]
const next = async () => {
...
.cleaner-onboarding { min-height: 100vh; padding-block: $space-8 $space-16; background: $color-background; &__container, form, fieldset { display: grid; gap: $space-6; } &__container { max-width: 64rem; } header h1 { margin-block: $space-3; font-size: $font-size-3xl; } fieldset { padding: 0; border: 0; } legend { margin-bottom: $space-4; font-size: $font-size-xl; font-weight: $font-weight-bold; } &__grid, &__checks, &__languages { display: grid; gap: $space-4; } &__checks label, &__languages label { display: flex; gap: $space-2; align-items: center; } &__languages { grid-template-columns: repeat(auto-fit, minmax(9rem, 1fr)); padding: $space-4; border: 1px solid $color-border; border-radius: $radius-md; } &__languages legend { padding-inline: $space-2; font-size: $font-size-md; } &__loading { color: $color-text-secondary; } &__actions { display: flex; gap: $space-3; & span { flex: 1; } } @media (min-width: $breakpoint-md) { &__grid, &__checks { grid-template-columns: repeat(2, minmax(0, 1fr)); } } }
</style>
