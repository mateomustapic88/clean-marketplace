<template>
  <header class="dashboard-header">
    <button type="button" class="dashboard-header__menu" :aria-label="t('owner.navigation.open')" @click="$emit('menu')">
    ...
    <NuxtLink class="dashboard-header__notifications" :to="notificationTo" :aria-label="t('notifications.title')">
      ...
      <span v-if="unreadCount" aria-hidden="true">{{ unreadCount > 99 ? '99+' : unreadCount }}</span>
    </NuxtLink>
    <LanguageSwitcher />
...
.dashboard-header {
  ...
  &__identity {
    ...
    div { display: grid; }
    small { color: $color-text-secondary; }
  }
